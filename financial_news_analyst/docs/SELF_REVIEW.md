...
**Decision**: All inference runs locally via Ollama (`qwen2.5:7b`), with no calls to OpenAI, Anthropic, or similar.

**Rationale**:
- Zero API cost, no rate limits, no data egress — all investment queries stay on the user's machine.
- Hardware available (2× RTX 4090, 48 GB VRAM total) makes local 7B–32B inference practical.
- GDPR-aligned: no user data sent to third parties.

**Trade-off**: Initial model quality is lower than GPT-4-class models. Mitigated by prompt engineering (structured task descriptions, current-date injection, tool-output rules) and RAG-grounded context. Upgrading to `qwen3:32b` (being pulled at the time of writing) will close most of this gap while keeping the local-only architecture.
...
5. **Model upgrade path**: The configuration is already parameterised (`AGENT_MODEL` env var). Switching to `qwen3:32b` requires only one line change — this was a deliberate design decision to future-proof the system.
...
| System Overview | ✅ Complete | `docs/SYSTEM_OVERVIEW.md` |
| Self-Review (this doc) | ✅ Complete | `docs/SELF_REVIEW.md` |
| Code Repository + README | ✅ Complete | GitHub: tbayan/capstone-project |
| Test Suite (95/96 passing) | ✅ Complete | `tests/` |
